...
extension String {
    ...
    func camelCase() -> String {
        ...
        let matches = regex.matches(in: trimed, options: [], range: NSRange(location: 0, length: trimed.characters.count))
        for m in matches.reversed() {
            let upper = (trimed as NSString).substring(with: m.rangeAt(3)).uppercased()
            (string as NSMutableString).replaceCharacters(in: m.rangeAt(1), with: upper)
        }
    ...
    func snakeCase() -> String {
        ...
        let string: NSMutableString = NSMutableString(string: baseString)
        let regex = try! NSRegularExpression(pattern: "((\\s)+(.))", options: .caseInsensitive)
        let matches = regex.matches(in: string as String, options: [], range: NSMakeRange(0, (string as String).characters.count))
        for m in Array(matches.reversed()) {
            let lower = "_" + (string as NSString).substring(with: m.rangeAt(3))
            (string as NSMutableString).replaceCharacters(in: m.rangeAt(1), with: lower)
        }
    ...
    func sanitizeAsMethodName() -> String {
        ...
        (string as NSMutableString).replaceOccurrences(of: ".", with: "_", options: [], range: NSMakeRange(0, self.characters.count))
        return NSString(string: string) as String
    }
}
